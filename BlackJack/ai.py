...
class Agent:
    ...
    def make_one_transition(self, action):
         #print("state:",self.simulator.state, "    action:",action)
         #print(self.simulator.state)

         #****I know supposed to return None but was giving me a headache so I made a work around***
         if action == HIT:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                trajectory.append((current_state,0))
                if next_state == (0,0,0):
                    trajectory.append((next_state,1))
                    break
            ...
            for i, (state, reward) in enumerate(reversed(trajectory)):
                G = DISCOUNT * G + reward
                if state not in trajectory[:len(trajectory)-i-1]:
                    self.N_MC[state] += 1
                    self.S_MC[state] += G
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                next_s, reward = self.make_one_transition(action)
                self.TD_values[s] += self.alpha(self.N_TD[s]) * (0 + DISCOUNT * self.TD_values[next_s] - self.TD_values[s])
                self.N_TD[s] +=1
                s = next_s
                if ( next_s == (0,0,0) or  next_s==(1,0,0)):
                    self.TD_values[s] += self.alpha(self.N_TD[s]) * (self.simulator.check_reward()  - self.TD_values[s])
                    self.N_TD[next_s] +=1
                    break
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                next_s, reward = self.make_one_transition(a)
                self.Q_values[s][a] += self.alpha(self.N_Q[s][a]) * (reward + DISCOUNT * max(self.Q_values[next_s]) - self.Q_values[s][a])
                self.N_Q[s][a] += 1
                s = next_s
                #idk why this code is not needed but cool
                #if ( next_s == (0,0,0) or  next_s==(1,0,0)):
                #    self.Q_values[s][a] += self.alpha(self.N_Q[s][a]) * (self.simulator.check_reward() + DISCOUNT * max(self.Q_values[s]) - self.Q_values[s][a])
                    #self.N_Q[next_s][a] += 1
                #    break
    ...
    def pick_action(self, s, epsilon):
        ...
        elif self.Q_values[s][0] > self.Q_values[s][1]:
            return 0
        else:
            return 1
