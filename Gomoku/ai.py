...
class AI:
    ...
    def select(self, node):
        ...
        while (not node.is_terminal): #correct
            if node.untried_actions: #correct if there is action expand
                return self.expand(node)#correct
            else:
                node ,_,_ = self.best_child(node, c=1)
        return node
    ...
    def best_child(self, node, c=1):
        ...
        for child in node.children:
            ...
            exploitation = child[1].num_wins / child[1].num_visits
            exploration = c* sqrt(2*log(node.num_visits) / child[1].num_visits)
    ...
    def backpropagate(self, node, result):
        ...
        while node is not None:
            node.num_visits += 1
            if (node.state[0] != winner):
                node.num_wins += result[winner]
            count += 1
            node = node.parent
    ...
    def rollout(self, node):
        ...
        while not self.simulator.game_over:
            ...
            self.simulator.place(move[0],move[1])
